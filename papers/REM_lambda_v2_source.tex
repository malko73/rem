...
The contiguous cuts are a small subset of the full factorisation manifold $\mathcal F \cong U(8)/(U(4)\times U(2))$, which has real dimension $44$. The present numerical implementation explores a four-parameter generator submanifold. We perform gradient ascent on $\Phi$ over this submanifold using anti-Hermitian generators and numerical central-difference gradients. The best recovered factorisation achieves
\[
\Phi_{\mathrm{continuous}} = 1.32,
\]
substantially exceeding the best contiguous cut ($\Phi=0.68$). This demonstrates that the discrete contiguous ansatz significantly constrains the search space; higher-scoring factorisations may lie outside the site-contiguous partitions.
