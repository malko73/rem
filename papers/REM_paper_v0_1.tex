...
a scaling law and not a thermodynamic limit.
...
boxed formula to first order. Here \(J_1\) is the first-order
\emph{coefficient} in the expansion (equivalently
\(dJ_\tau/d\tau|_0 = J_1\)); we use the coefficient convention
throughout. The first-order coefficient is computed both numerically
(small-\(\tau\) slope) and analytically. With the fixed Schmidt basis
\(Q_F\), \(C^2(\tau) = |Q_F U^\dagger e^{\tau L}\rho\, U|^2\) and
