...
demonstrate numerically that singularity, product collapse, and seed
instability disappear. The selected factorization responds
systematically to both the Hamiltonian and the state (cross-evaluation
matrix diagonal dominance). Extending the functional to a finite
observation window \(\tau\) reveals \textbf{competing structural
basins}: the instantaneously optimal factorization and a distinct
finite-time-optimal factorization cross at \(\Delta\Phi(\tau_c) = 0\),
with \(\tau_c = 0.0180, 0.0206, 0.0224, 0.0217\) for \(N = 3,4,4,5\)
(cuts \(2|1\), \(2|2\), \(1|3\), \(2|3\)) --- approximately flat over
the tested finite-size range. The crossover scale is \textbf{derived},
not fitted: to first order in the finite-time expansion
...
\emph{fixed-\((F)\) local diagnostic only; not an unrestricted global
TPS variational objective} (Spec v2.2 §7).
