...
near-product directions. The mechanism is general for pure states (see
below), and the numerical evidence demonstrates it in the tested
setting.}
...
\item
  ...
  linear interpolation of the sign change (Fig. 2; full 19-point grid
  \(\tau \in [0.003, 0.1]\) in Supplementary Table S2 --- the table
  shows selected points). \(\Delta\Phi\) is evaluated at fixed
  representatives and is therefore deterministic; the optimizer seed
  spread is reported as the per-\(\tau\) std of \(\Phi\) across the 6
  seeds.
\item
...
value and is framed as a finite-\(N\) observation.
...
negative --- the crossing curve bends back at larger \(\tau\),
consistent with the non-monotone \(\Delta\Phi\) seen in D4); the fit
uncertainties are tiny (\(\Delta\Phi_0 \pm 10^{-4}\),
\(\Delta\Phi_1 \pm 10^{-3}\)), so the quoted \(\tau_c^{(1)}\) values are
numerically well-determined at the fixed representatives; re-evaluating
\(\tau_c^{(1)}\) with alternative representatives \(F_B\) (second/third
seed, or the stored \(\tau=0.05\) optimum for \(N=3\)) shifts it by
\(\lesssim 4\times10^{-3}\); and the measured \(\tau_c\) comes from
linear interpolation on a grid whose local spacing is \(2\times10^{-3}\)
(\(N=3\)) or \(10^{-2}\) (\(N=4,5\)), so the measured values carry an
interpolation resolution of at most \(\pm 5\times10^{-3}\). ``Derived to
first order'' therefore means: \(\tau_c^{(1)}\) is predicted
analytically from \(t=0\) operator matrix elements at fixed basin
configurations \((F_A,F_B)\) --- it is not fitted to the time-dependent
curve \(\Delta\Phi(\tau)\) --- while the representative choice and the
grid resolution contribute a documented uncertainty of a few
\(\times10^{-3}\). The quadratic correction \(\tau_c^{(2)}\), by
contrast, is a polynomial fit and is presented as a consistency check
only.
...
\bottomrule\noalign{}
\endlastfoot
\(\Delta\Phi\) & −0.0214 & −0.0199 & −0.0185 & −0.0170 & −0.0155 &
−0.0141 & −0.0126 & −0.0112 & −0.0083 & −0.0055 \\
\(\tau\) & 0.016 & 0.018 & 0.020 & 0.025 & 0.030 & 0.040 & 0.050 & 0.070
& 0.100 & \\
\(\Delta\Phi\) & −0.0028 & −0.0000 & +0.0027 & +0.0092 & +0.0155 &
+0.0275 & +0.0385 & +0.0576 & +0.0794 & \\
\end{longtable}
}
